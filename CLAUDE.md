...
- Browserless integration (BROWSERLESS_TOKEN, BROWSERLESS_URL)
...
n8n is configured with `N8N_BLOCK_ENV_ACCESS_IN_NODE: false`, allowing Code nodes to access environment variables. This enables integrations via env vars such as:
- `BROWSERLESS_TOKEN` - Authentication token for Browserless API
- `BROWSERLESS_URL` - Browserless service endpoint (default: `http://browserless:3000`)
...
- n8n has `N8N_BLOCK_ENV_ACCESS_IN_NODE: false` to allow Code nodes to read env vars (e.g., BROWSERLESS_TOKEN)
- Browserless is configured via environment variables, not as a bundled container
