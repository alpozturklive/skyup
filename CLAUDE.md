...
SkyUp is a self-hosted application stack (Release v2.2 - Streamlined) that runs AI applications and workflow tools using **Podman** and **podman-compose**. It's designed for host-only access (127.0.0.1) with optional HTTPS setup for production.
...
- n8n basic auth credentials (N8N_BASIC_USER, N8N_BASIC_PASSWORD)
A `.env.example` file is provided as a template for all required variables.

The init script preserves existing passwords when re-run but **deletes and recreates** PostgreSQL and n8n data directories (`.podman/pgvector-varlibpostgresqldata`, `.podman/n8n-homenode.n8n`). Open WebUI data is preserved.
...
n8n is configured with `N8N_BLOCK_ENV_ACCESS_IN_NODE: false`, allowing Code nodes to read environment variables from within n8n workflows.

### n8n Webhook / Domain Configuration

The n8n service in `podman-compose.yml` has domain settings **hardcoded to `skyup.online`**:
...
These must be updated before starting the stack when deploying to a different domain. After changing, run `podman-compose restart n8n`.
...
- The `memory` database is available for n8n workflows to store chat memory and persistent data
- Ollama uses Vulkan for GPU acceleration (OLLAMA_VULKAN=1)
- All services bind to 127.0.0.1 for host-only access except Nginx (0.0.0.0 for production HTTPS)
- n8n has `N8N_BLOCK_ENV_ACCESS_IN_NODE: false` to allow Code nodes to read environment variables
