...
**Current Release:** `Release v2.2 - Streamlined`
...
**What's New in v2.2:**
- Enabled `N8N_BLOCK_ENV_ACCESS_IN_NODE: false` so n8n Code nodes can access environment variables
- Fixed n8n port binding to `127.0.0.1:5678` (host-only access)
- Fixed `.env.example` filename and added missing variables
- Updated all documentation to match current stack configuration
...
./init-env.sh
```
This script will create strong, random passwords for your services and store them in the `.env` file. You can also reference `.env.example` for a template of all required variables.

> **Warning:** Running `init-env.sh` deletes and recreates the PostgreSQL and n8n data directories (`.podman/pgvector-varlibpostgresqldata` and `.podman/n8n-homenode.n8n`). Only Open WebUI data is preserved. Back up before re-running if you have existing data.
...
n8n is configured with `N8N_BLOCK_ENV_ACCESS_IN_NODE: false`, which allows Code nodes to read environment variables from within Code nodes.

### n8n Webhook / Domain Configuration

The `podman-compose.yml` file has the following n8n variables hardcoded to `skyup.online`:
...
If you are using a different domain, update these values in `podman-compose.yml` before starting the stack, then restart n8n: `podman-compose restart n8n`.
...
2. **Update n8n domain variables** in `podman-compose.yml`:
   ```yaml
